/**
 * 拉丝三点：乘法 = combo 优先，否则胜方两人 personalMul 相乘。
 * 运行：node scripts/lasuoThreePointMul.selftest.js
 */
...
assert(
  'source: three-point 早退复用 resolveComboThenPersonalProduct',
...
var hole7 = { A: -2, B: 1, C: 2, D: 0 };
var sum7 = settle(hole7, { pkTotalMode: 'sum' });
var prod7 = settle(hole7, { pkTotalMode: 'product' });
assert(
  'CASE7 pkTotalMode=product 只改比较；倍率仍是 personal product 不是洞分积',
  sum7.winningTeam === 'A' &&
    prod7.winningTeam === 'A' &&
    sum7.multiplierSource === 'personal-product' &&
    prod7.multiplierSource === 'personal-product' &&
    Number(sum7.multiplier) === 5 &&
    Number(prod7.multiplier) === 5 &&
    Number(prod7.multiplier) !== 8,
