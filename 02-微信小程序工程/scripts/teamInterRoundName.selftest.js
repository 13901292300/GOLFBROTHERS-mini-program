/**
 * 普通队际创建/编辑：roundName 14 字、blur 规范化、必填
 * 运行：node scripts/teamInterRoundName.selftest.js
 */
...
assert(
  '新建 lastOk 为默认自动名',
  !!page._roundNameLastOk && page._roundNameLastOk === page.data.roundName
);
...
page = makePage();
page.onRoundNameInput({ detail: { value: '  队际联谊  ' } });
assert(
  'bindinput 不 trim',
  page.data.roundName === '  队际联谊  '
);
assert(
  'blur 清换行并 trim 且写回页面',
  page.onRoundNameBlur({ detail: { value: ' \n 队际联谊 \r ' } }) === true &&
    page.data.roundName === '队际联谊' &&
    page._roundNameLastOk === '队际联谊'
);
...
var autoName = policy.buildAutoInterRoundName([
  { name: '前海国际' },
  { name: '深圳湾' },
  { name: '华侨城' },
  { name: '香蜜湖' }
...
assert(
  '自动标题≤14',
  policy.countTitleChars(autoName) <= 14 && autoName.indexOf('联谊赛') >= 0
);
...
page.setData(page._applyAutoRoundNameIfNeeded({}, groupsA));
var autoAfterTeams = page.data.roundName;
assert(
  '自动名写入同步 lastOk',
  page._roundNameLastOk === autoAfterTeams && autoAfterTeams.indexOf('联谊赛') >= 0
);
page.onRoundNameInput({ detail: { value: '手工比赛名' } });
page.onRoundNameBlur({ detail: { value: '手工比赛名' } });
page.setData(
  page._applyAutoRoundNameIfNeeded(
    {},
    [{ name: '华侨城' }, { name: '香蜜湖' }]
  )
);
...
console.log('');
console.log('teamInterRoundName.selftest: ' + passed + ' passed, ' + failed + ' failed');
if (failed) {
