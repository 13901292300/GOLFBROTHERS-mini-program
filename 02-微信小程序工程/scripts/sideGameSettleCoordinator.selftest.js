...
global.__gb_side_games = [makeRecord('lasuo-4')];
var input = officialInput(scores5);
coord.settleSideGamesForScoreMutation(input);
var afterFirst = freeze(global.__gb_side_games[0]);
...
assert(
  ...
  freeze(global.__gb_side_games[0]).resultRevision === afterFirst.resultRevision &&
    freeze(global.__gb_side_games[0]).resultSnapshot === afterFirst.resultSnapshot &&
    freeze(global.__gb_side_games[0]).orderByHole === afterFirst.orderByHole
);
...
var beforeTab = freeze(global.__gb_side_games[0]);
projectMod.project(input);
assert('CASE4 切 TAB 不 settle', coord.getSettleCallCount() === 1 && freeze(global.__gb_side_games[0]).resultRevision === beforeTab.resultRevision);

var nextLabel = catalog.HOLES[projectMod.lastCompleteHoleIndex(input) + 1];
var beforeNext = (global.__gb_side_games[0].resultSnapshot.orderByHole || {})[nextLabel];
var changed = {
  pA: filledScores(5),
  pB: filledScores(5),
  pC: filledScores(5),
...
var afterPush = freeze(global.__gb_side_games[0]);
var storageNext = (global.__gb_side_games[0].resultSnapshot.orderByHole || {})[nextLabel];
...
coord.resetSettleCallCount();
global.__gb_side_games = [makeRecord('8421-4')];
var input8421 = officialInput(scores5);
coord.settleSideGamesForScoreMutation(input8421);
assert('CASE7 8421 coordinator settle', coord.getSettleCallCount() === 1 && (global.__gb_side_games[0].resultRevision || 0) === 1);
...
assert('CASE7 8421 再 settle revision+1', (global.__gb_side_games[0].resultRevision || 0) === 2);
