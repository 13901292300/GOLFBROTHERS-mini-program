...
var seriesStoreMod = require('../../../utils/seriesStore.js');
var seriesStationIndexMod = require('../../../utils/seriesStationIndex.js');
...
function createMemoryMatchRepo(options) {
  ...
  return {
    ...
    removeMatch: function (matchId) {
      var mid = asString(matchId).trim();
      if (!mid) return { ok: false, reason: 'match_id_required' };
      if (!map[mid]) return { ok: true, reason: 'absent' };
      delete map[mid];
...
function createSeriesPublisher(deps) {
  ...
  var matchRepo = d.matchRepo || {
    ...
    removeMatch: function (id) {
      var mid = asString(id).trim();
      if (!mid) return { ok: false, reason: 'match_id_required' };
      var existed = !!teamMatchStore.getMatchById(mid);
      teamMatchStore.removeMatch(mid);
      return { ok: true, reason: existed ? 'removed' : 'absent' };
    }
  };
  var nowFn =
  ...
  function publishSeries(seriesId, options) {
    ...
    if (!journal) {
      ...
      if (asString(journal.phase).trim() === 'discarding') {
        return {
          ok: false,
          reason: 'discard_in_progress',
          journal: journal,
          series: series,
          message: '正在放弃本次创建，请再次点击取消以继续清理'
        };
      }
  ...
  function inspectPublishState(seriesId) {
    ...
    var canRetry =
      hasFrozenPlan &&
      !sourceDrift &&
      asString(journalPhase) !== 'discarding' &&
  ...
  var STATION_CONFLICT_MESSAGE = '存在冲突分站，无法自动取消';

  function resolveDiscardActorId(options) {
    var opts = options || {};
    if (opts.actorUserId != null && asString(opts.actorUserId).trim()) {
      return asString(opts.actorUserId).trim();
    }
    var actor = opts.actor && typeof opts.actor === 'object' ? opts.actor : {};
    return asString(actor.userId || actor.playerId || opts.creatorId).trim();
  }

  function resolveSeriesCreatorId(series, journal) {
    if (series && asString(series.createdBy).trim()) {
      return asString(series.createdBy).trim();
    }
    var rounds = journal && Array.isArray(journal.rounds) ? journal.rounds : [];
    var i;
    for (i = 0; i < rounds.length; i++) {
      var payload = rounds[i] && rounds[i].matchPayload;
      var cid = asString(payload && (payload.createdBy || payload.creatorId)).trim();
      if (cid) return cid;
    }
    return '';
  }

  function findRoundPlan(journal, matchId) {
    var mid = asString(matchId).trim();
    var rounds = Array.isArray(journal && journal.rounds) ? journal.rounds : [];
    var i;
    for (i = 0; i < rounds.length; i++) {
      if (asString(rounds[i] && rounds[i].matchId).trim() === mid) return rounds[i];
    }
    return null;
  }

  function inspectOwnedStation(journal, rp) {
    var mid = asString(rp && rp.matchId).trim();
    var rid = asString(rp && rp.roundId).trim();
    var existing = matchRepo.getMatchById(mid);
    if (existing) {
      var ctx = existing.seriesContext || {};
      var sameIdentity =
        ctx.managed === true &&
        asString(ctx.seriesId).trim() === asString(journal.seriesId).trim() &&
        asString(ctx.roundId).trim() === rid &&
        asString(ctx.publishToken).trim() === asString(journal.publishToken).trim() &&
        asString(existing.matchId).trim() === mid;
      if (!sameIdentity) {
        return {
          ok: false,
    ...
    return {
      ...
      matchId: mid,
      roundId: rid,
      matchPresent: !!existing,
      indexPresent: !!link
    };
  }

  function inspectInterruptedPublishDiscard(seriesId, options) {
    var sid = asString(seriesId).trim();
    if (!sid) return { ok: false, reason: 'series_id_required', canDiscard: false };
    var actorId = resolveDiscardActorId(options);
    if (!actorId) return { ok: false, reason: 'actor_required', canDiscard: false };
    var series = loadSeries(sid);
    var jRes = journalStore.getJournal(sid);
    if (!jRes.ok) {
      return { ok: false, reason: jRes.reason || 'journal_read_failed', canDiscard: false };
    }
    var journal = jRes.journal;
    if (!journal) {
      return { ok: false, reason: 'no_frozen_plan', canDiscard: false, series: series };
    }
    var jv = seriesStationMatch.validateFrozenJournal(journal);
    if (!jv.ok) {
      return {
    ...
    if (series && asString(series.lifecycleStatus).trim() === 'published') {
      return {
        ok: false,
        reason: 'published_not_allowed',
        canDiscard: false,
        series: series,
        journal: journal,
        message: '系列赛已创建，不能从创建页取消'
      };
    }
    var creatorId = resolveSeriesCreatorId(series, journal);
    ...
    var i;
    for (i = 0; i < rounds.length; i++) {
      var row = inspectOwnedStation(journal, rounds[i]);
      if (!row.ok) {
        return {
          ok: false,
          reason: 'station_conflict',
          ...
          series: series,
          journal: journal,
          message: STATION_CONFLICT_MESSAGE
        };
      }
      owned.push(row);
    }
  ...
  function discardInterruptedPublishSafely(seriesId, options) {
    var inspected = inspectInterruptedPublishDiscard(seriesId, options);
    if (!inspected.ok || !inspected.canDiscard) return inspected;
    var journal = deepClone(inspected.journal);
    var series = inspected.series;
    var sid = asString(journal.seriesId).trim();
    journal.phase = 'discarding';
    journal.lastError = '';
    var marked = persistJournal(journal);
    if (!marked.ok) {
      return {
        ok: false,
        reason: 'journal_write_failed',
    ...
    var owned = inspected.owned || [];
    var i;
    for (i = 0; i < owned.length; i++) {
      var idxRow = owned[i];
      var liveLink = stationIndex.getByMatchId(idxRow.matchId);
      if (!liveLink) continue;
      if (
        asString(liveLink.seriesId).trim() !== sid ||
        asString(liveLink.roundId).trim() !== asString(idxRow.roundId).trim()
      ) {
        return {
          ok: false,
          reason: 'station_conflict',
          detail: 'index_conflict',
          matchId: idxRow.matchId,
          journal: journal,
          series: series,
          message: STATION_CONFLICT_MESSAGE
        };
      }
      var rmIdx = stationIndex.removeByMatchId(idxRow.matchId);
      if (!rmIdx || !rmIdx.ok) {
        return {
          ok: false,
          reason: (rmIdx && rmIdx.reason) || 'index_failed',
          matchId: idxRow.matchId,
          journal: journal,
          series: series
        };
    ...
    for (i = 0; i < owned.length; i++) {
      var matchRow = owned[i];
      var live = matchRepo.getMatchById(matchRow.matchId);
      if (!live) continue;
      var rp = findRoundPlan(journal, matchRow.matchId);
      var again = inspectOwnedStation(journal, rp || matchRow);
      if (!again.ok) {
        return {
          ok: false,
          reason: 'station_conflict',
      ...
      if (typeof matchRepo.removeMatch !== 'function') {
        return {
          ok: false,
          reason: 'match_remove_unavailable',
          journal: journal,
          series: series
        };
      }
      var rm = matchRepo.removeMatch(matchRow.matchId);
      if (!rm || !rm.ok) {
        return {
          ok: false,
          reason: (rm && rm.reason) || 'match_remove_failed',
          matchId: matchRow.matchId,
          journal: journal,
          series: series
        };
    ...
    if (loadSeries(sid)) {
      var rmSeries = seriesStore.removeDraft(sid);
      if (!rmSeries || !rmSeries.ok) {
        if (!(rmSeries && rmSeries.reason === 'not_found')) {
          return {
            ok: false,
            reason: (rmSeries && rmSeries.reason) || 'series_remove_failed',
            journal: journal
          };
        }
      }
    ...
    if (typeof journalStore.removeJournal !== 'function') {
      return { ok: false, reason: 'journal_remove_unavailable', journal: journal };
    }
    var rmJ = journalStore.removeJournal(sid);
    if (!rmJ || !rmJ.ok) {
      return {
        ok: false,
        reason: (rmJ && rmJ.reason) || 'journal_remove_failed',
        journal: journal
      };
    }
    return {
      ...
      reason: 'discarded',
      seriesId: sid,
      removedMatchIds: owned.map(function (r) {
        return r.matchId;
      })
    };
  }
  ...
  return {
    ...
    inspectInterruptedPublishDiscard: inspectInterruptedPublishDiscard,
    discardInterruptedPublishSafely: discardInterruptedPublishSafely,
...
module.exports = {
  ...
  inspectInterruptedPublishDiscard: function (seriesId, options) {
    return getDefaultPublisher().inspectInterruptedPublishDiscard(seriesId, options);
  },
  discardInterruptedPublishSafely: function (seriesId, options) {
    return getDefaultPublisher().discardInterruptedPublishSafely(seriesId, options);
  },
