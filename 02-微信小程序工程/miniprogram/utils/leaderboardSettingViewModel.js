...
function buildLeaderboardSettingViewModel(match, currentSelection, opts) {
  ...
  var viewOpts = [];
  if (viewOptions.indexOf('team') >= 0) {
    viewOpts.push({ key: 'team', name: sideLabel, sub: '', disabled: false });
  }
  viewOpts.push(
    { key: 'all', name: '全部', sub: '', disabled: false },
    { key: 'male', name: '男子', sub: '', disabled: false },
    { key: 'female', name: '女子', sub: '', disabled: false }
  );
  var sections = [
    {
      key: 'scoreType',
